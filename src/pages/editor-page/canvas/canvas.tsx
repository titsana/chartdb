...
export const Canvas: React.FC<CanvasProps> = ({ initialTables }) => {
    ...
    const handleViewportMove = useCallback(
        (_event: MouseEvent | TouchEvent | null, viewport: Viewport) => {
            if (awareness && !viewportRafIdRef.current) {
                viewportRafIdRef.current = requestAnimationFrame(() => {
                    awareness.setLocalStateField(
                        'viewportCenter',
                        viewportToCenter(viewport, flowWidth, flowHeight)
                    );
                    viewportRafIdRef.current = undefined;
                });
            }
        },
    ...
    // pan/zoom (or a programmatic setViewport/setCenter), so a peer who
    // hasn't touched the canvas since loading has no `viewportCenter`
    // field at all yet. Without this, clicking their avatar to follow them
    // does nothing until they happen to move.
    useEffect(() => {
        if (!awareness) return;
        awareness.setLocalStateField(
            'viewportCenter',
            viewportToCenter(getViewport(), flowWidth, flowHeight)
        );
    }, [awareness, getViewport, flowWidth, flowHeight]);
